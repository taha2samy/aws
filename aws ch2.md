...
>
> النقطة الـ crucial هنا: **الـ Target Group نفسها مبتعملش Health Checks**. هي مجرد container للإعدادات بتاعة الـ Health Check. اللي بيعمل الـ Health Checks ده هو الـ **Load Balancer** (ALB، NLB، أو CLB) اللي مرتبط بـ Target Group معينة. هو اللي بيبقى active وبيعمل الـ Checks دي regularly على كل الـ Registered Targets اللي جوه الـ Target Group دي. Based on نتايج الـ Checks دي، الـ Load Balancer بيقرر إذا كان الـ Target (مثلاً EC2 instance) "Healthy" يبقى ready إنه يستقبل الـ Traffic، ولا "Unhealthy" 
>
...
انت هنا مش بتستخدام cloudwatch بشكل مباشر
...
![Systems Manager icon](assets/Systems-Manager.svg+xml)
...
ب لو الـ `Patch Manager` وظيفته إنه يتأكد إن كل الـ `instances` بتاعتك على نفس الـ `patch level` (يعني واخدين نفس التحديثات الأمنية)، فالـ `State Manager` ده بقى حاجة أوسع بكتير. هو يعتبر أداة لـ **`configuration management`** (إدارة الإعدادات). مهمته الرئيسية إنه يضمنلك إن الـ `instances` اللي عندك مش بس متحدثة، لأ دي كمان عندها السوفتوير اللي أنت عايزه بالظبط، ومتظبطة بنفس الإعدادات والطريقة اللي أنت محددها.
...
أول ما بتعمل الـ `association` دي، الـ `State Manager` بيقوم بتنفيذها على طول `immediately` (فوراً) على كل الـ `target instances` اللي تبقى `online` (متصلة وشغالة). وبعد أول مرة، بيفضل يتابع الـ `schedule` اللي أنت حاطه وينفذ الـ `command` أو الـ `policy` ده بانتظام.
...
*   `file information` 
*   `CPU information` 
*   ولو الـ `instance` دي `Windows`، بيقدر يجمع كمان `registry values` (قيم الـ Registry).

**الـ Insights :**

، الـ `Insights` دي  لوحة تحكم كده أو مكان واحد بيجمعلك كل المعلومات المهمة عن الـ `AWS resources` (الموارد) بتاعتك. هي بتعمل `aggregate` (تجميع) لكل تفاصيل الـ `health` ، والـ `compliance` (مدى التزامها بالسياسات والإعدادات اللي أنت محددها)، وكمان الـ `operational details`. كل ده بتلاقيه في مكان واحد بس جوه `AWS Systems Manager`.

بعض الـ `Insights` دي بتبقى متصنفة ومتقسمة حسب **`AWS resource groups`** ( إيه هي الـ `resource groups` دي؟ دي ببساطة كده تجمعات أنت بتعملها للموارد بتاعتك اللي موجودة في `AWS region` (منطقة AWS) واحدة.
...
*   `EC2 instances` (سيرفرات التطبيق).
*   `S3 buckets` (أماكن تخزين الملفات).
*   `EBS volumes` (مساحات التخزين المربوطة بالسيرفرات).
*   `security groups`
*   وهكذا...
...
*   وتحطلها **نفس الـ `tag value`** (مثلاً تسميها `MyWebsiteApp`).
    ...
    *   الـ `S3 bucket` هيكون عنده `Tag`:
         `Key: Application`, 
        `Value: MyWebsiteApp`.
    *   وهكذا لكل الموارد اللي تبع نفس التطبيق.

الـ `Insights` بقى بتاخد التجمعات دي (`resource groups`)، وبتصنفلك المعلومات اللي بتجيبها على أساسها. زي ما بيقولوا كده، الـ `Insights` دي بتيجي متصنفة، وهما هيغطوا تصنيفاتها بالتفصيل في الجزء اللي بعده. لكن الفكرة الأساسية إنها بتديلك رؤية شاملة ومنظمة لحالة مواردك بناءً على تقسيمات أنت بتحددها بنفسك.
...
Built-in Insights
 ...
 . دي باختصار كده، **لوحات مراقبة أو شاشات متابعة، Systems Manager بيوفرها لك بشكل تلقائي** من غير ما تتعب نفسك وتعملها إنت. دي بتديك فكرة سريعة ومفيدة عن اللي بيحصل في حسابك على AWS.

التحليلات الجاهزة دي بتشمل الآتي:

*   **AWS Config Compliance (تطابق إعدادات AWS):**
    *   دي بتوريك يا معلم، كام مورد (Resource) عندك في مجموعة معينة (زي مثلاً كام سيرفر، كام قاعدة بيانات) مطابِق للقواعد اللي إنت حاططها في AWS Config، وكام واحد مش مطابِق. يعني لو قايل: "كل السيرفرات لازم تكون مشفرة"، دي توريك مين ماشي صح ومين لأ.
    *   وبتعرضلك كمان تاريخ مختصر بالتغييرات اللي حصلت في إعدادات الموارد دي، وكل ده AWS Config بتسجله أول بأول.

*   **CloudTrail Events (أحداث CloudTrail):**
    *   التحليلة دي بتعرضلك كل مورد في المجموعة بتاعتك، بتقولك نوعه إيه (يعني ده Bucket ولا سيرفر ولا إيه بالظبط)، وآخر حاجة حصلت عليه وسجلها CloudTrail.
    *   يعني مثلاً، لو حد مسح ملف من S3 Bucket، دي توريك إن الـ Bucket ده آخر حاجة حصلت عليه كانت "مسح ملف" (DeleteObject) وتعرف مين اللي عمل كده وإمتى.

*   **Personal Health Dashboard (لوحة صحة حسابك الشخصية):**
    *   دي بقى زي "عيادة الطوارئ" بتاعت حسابك في AWS. بتديلك تنبيهات سريعة لو AWS نفسها عندها مشكلة ممكن تأثر على الموارد بتاعتك.
    *   مثلاً، لو فيه خدمة من خدمات AWS (زي S3 أو EC2) حصل فيها بطء بسيط أو مشكلة فنية، الداشبورد دي بتنبهك عشان تاخد بالك.
    *   وبتوريك كمان كام مشكلة AWS حلتها وظبطتها في آخر 24 ساعة.

*   **Trusted Advisor Recommendations (نصايح المستشار الموثوق):**
    *   دي بقى عاملة زي "الخبير اللي بيديك نصايح" لحسابك في AWS. بيراجع البيئة بتاعتك كلها ويديلك اقتراحات عشان تحسن حاجات معينة:
        *   **تقلل مصاريفك (Cost Optimization):** إزاي توفر فلوس.
        *   **تحسن الأداء (Performance):** إزاي تخلي حاجتك تشتغل أسرع.
        *   **تزود الأمان (Security):** إزاي تحمي حاجتك أكتر.
        *   **تزود تحمل الأعطال (Fault Tolerance):** إزاي تخلي حاجتك متقعش بسهولة لو حصلت مشكلة.
    ...
    *   لكن كل العملاء التانيين (حتى اللي مش دافعين لدعم زيادة)، AWS بتديلهم شوية فحوصات أمنية مجانية مهمة جداً، زي:
        *   **لو S3 Bucket بتاعك مفتوح على العام:** خصوصاً لو أي حد يقدر يرفع عليه ملفات (upload) أو يمسح منه (delete)، دي بتبقى خطيرة جداً وTrusted Advisor بينبهك ليها.
        *   **لو Security Groups عندك مفتوح فيها بورتات خطيرة على العام:** زي بورتات قواعد البيانات (TCP port 1433 لـ MySQL) أو بورتات الدخول عن بعد (3389 لـ Remote Desktop Protocol). دي المفروض تكون مقفولة ومحدش يوصلها غير ناس معينة، ولو مفتوحة Trusted Advisor بيحذرك.
        ...
        *   **لو EBS أو RDS Snapshots بتاعتك مفتوحة على العام:** لو عندك نسخ احتياطية (صور أو Snapshots) من وحدات تخزين (EBS) أو قواعد بيانات (RDS) ومسموح لأي حد يوصلها بشكل عام، دي برضه خطيرة جداً وبيتم التنبيه عليها.
...
![AWS Systems Manager Inventory](assets/Res_AWS-Systems-Manager_Inventory_48_Light.svg+xml)
...
*   يا إما تختار الـ Instances عن طريق **الـ Tags** اللي إنت حاططها عليها (الـ Tags دي زي ليبلز كده بتعلقها على الـ Instances عشان تصنفها).
...
بص يا معلم، الـ **Kubernetes (واختصارها المشهور K8s)**، دي أداة "Open Source"  عملتها جوجل في الأصل عشان تستخدمها هي بنفسها جواها. دورها الأساسي هو إنها "Container Orchestrator" 
